...
const DOCUSEAL_API_KEY =
  process.env.DOCUSEAL_API_KEY || "your-docuseal-api-key";
...
/**
 * Create a DocuSeal submission from a PDF template or HTML
 * Callable function that takes template data, signer info, and creates a submission
 */
exports.createDocuSealSubmission = onCall(
  {
    region: "us-central1",
  ...
  async (request) => {
    try {
      ...
      if (templateId) {
        ...
        if (redirectUrl) {
          submissionData.redirect_url = redirectUrl;
        }
      } else if (templateHtml) {
        // Create template from HTML and then submission
        const templateResponse = await axios.post(
          `${DOCUSEAL_API_BASE}/templates/html`,
          {
            name: templateName || "Document Template",
            html: templateHtml,
            fields: fields,
          },
          {
        ...
        throw new Error("Either templateId or templateHtml must be provided");
      }
      ...
      await admin
        ...
        .set({
          ...
          templateName: submission.template?.name || templateName,
      ...
      logger.info("DocuSeal submission created successfully", {
        submissionId: submission.id,
        submissionUrl: submission.url,
      });
      ...
      logger.error("Error creating DocuSeal submission", {
        error: error.message,
        stack: error.stack,
        userId: request.auth?.uid,
      });
